	...
	overwriteLine = NULL,...) 
{
    ...
    if (style == "gap") {
        ...
        if(fill) rect(br[1], br[2], br[3], br[4], col = bgcol, border = bgcol,xpd=NA)
        if (style == "slash") {
            if (axis == 1 || axis == 3) {
				...
				if(! is.null(overwriteLine)){
				   owl.base <- mean(br[c(2,4)]);
				   owl.start <- breakpos - xw/2;
				   owl.end   <- breakpos + xw/2;
				   owl.list  <- c(list(x0 = owl.start, y0 = owl.base, x1 = owl.end, y1 = owl.base,xpd=NA),overwriteLine);
				   do.call(segments,owl.list);
				}
            }
            else {
                xbegin <- c(br[1], br[1])
				...
				if(! is.null(overwriteLine)){
				   owl.base <- mean(br[c(1,3)]);
				   owl.start <- breakpos - yw/2;
				   owl.end   <- breakpos + yw/2;
				   owl.list  <- c(list(x0 = owl.base, y0 = owl.start, x1 = owl.base, y1 = owl.end,xpd=NA),overwriteLine);
				   do.call(segments,owl.list);
				}
            }
        }
