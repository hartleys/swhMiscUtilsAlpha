...
#++PARAMS="--emitAnyNonmissing: For use with tableGT, with this option only samples with any non-reference allele will have a line."$'\n'
...
lineLimit=-1
while "--lineLimit" in args:
   idx = args.index("--lineLimit");
   lineLimit=int(args.pop(idx+1));
   args.pop(idx);
...
emitAnyNonmissing=False;
if "--emitAnyNonmissing" in args:
   idx = args.index("--emitAnyNonmissing");
   keepGT=True;
   emitAnyNonmissing=True;
   filterGeno=True;
   args.pop(idx);
...
if "--gtStyle" in args:
   idx = args.index("--gtDelim");
   args.pop(idx);
   gtStyle= args.pop(idx);
   if gtStyle not in ["GT","01"]:
       eprint("ERROR: gtStyle parameter must be either 'GT' or '01'");
      ...
      if len(line) == 1:
         ...
         if keepGT:
            if tableGT :
               ...
               if emitOnlySimpleHet or emitOnlyNonref or emitAnyNonref or filterGeno or emitOnlySamplesOnList or emitAnyNonmissing:
                  out.write("\tFORMAT");
         ...
         if lineLimit > 0 and lnct > lineLimit:
            eprint("BREAKING at lineLimit="+str(lineLimit))
            break;
         if lnct % 1000 == 0:
           eprint(".", end='');
           if lnct % 5000 == 0:
