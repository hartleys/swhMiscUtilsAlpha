...
import sys,gzip,errno,time,re,os
...
#+ HELPTEXT="vcfheadertable.py utility. Takes the header data and makes a table."$'\n'
#++HELPTEXT=" "$'\n'
...
#++PARAMS="--noFmt: Do not include FORMAT field listings"$'\n'
#++PARAMS="--noInfo: Do not include INFO field listings"$'\n'
#++PARAMS="--linePrefix <prefix> or -p <prefix>: Append <prefix> string to each line"$'\n'
#++PARAMS="--closeQuote: ..."$'\n'
#++PARAMS="--openQuote: ..."$'\n'
#+ VERSION="0.0.5"
...
infileBasenameNoExt = re.sub(".gz$","",infileBasename)
infileBasenameNoExt = re.sub(".vcf$","",infileBasename)
outfile = infileBasenameNoExt + ".header.txt";
outfilenum = 1;
while os.path.exists(outfile):
   outfilenum = outfilenum + 1;
   outfile = infileBasenameNoExt +".v"+str(outfilenum) + ".header.txt";
...
if "--noHashPrefix" in args:
   idx = args.index("--noHashPrefix")
   args.pop(idx);
   linePrefix="";

if "--linePrefix" in args or "-p" in args:
   if "--linePrefix" in args:
      idx = ( args.index("--linePrefix"));
   else:
      idx = ( args.index("-p")); 
   args.pop(idx);
   linePrefix=args.pop(idx);
      ...
      if len(line) == 1:
         ...
         if writeINFO:
            ...
            for infoTag in infoCols:
               ...
               out.write(linePrefix + infoTag+"\t"+md[1]+"\t"+md[2]+"\t"+md[3]+"\n");
         if writeINFO and writeFMT:
            out.write("###--------------------------------------\n");
            out.write("###FORMAT TAGS: \n")
         if writeFMT:
            out.write("#fmtTag\tnumber\ttype\tdescription\n");
            for fmtTag in fmtCols:
               md = fmtMetaData[fmtTag]
               out.write(linePrefix + fmtTag+"\t"+md[1]+"\t"+md[2]+"\t"+md[3]+"\n");
         if writeSamps:
            if len(line) >= 10:
               out.write( linePrefix + ("\n" + linePrefix).join( line[:-1].split("\t")[9:] ) +"\n");
      elif line[0:7] == "##INFO=":
