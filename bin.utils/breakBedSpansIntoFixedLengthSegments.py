    ...
    if line[0:5] != "chrom":
        ...
        if lnct % 10000 == 0:
           ...
           if lnct % 50000 == 0:
              eprint(" ",end='');
           if lnct % 100000 == 0:
              eprint("["+str(lnct)+" lines complete] ["+time.strftime("%c")+"] ["+cells[0]+":"+cells[1]+" "+cells[2]+"]")
        start = int(cells[1])
        end   = int(cells[2]);
        span = end - start;
        partct = 1;
        while span > MAXSPAN:
            out.write(cells[0]+"\t"+str(start)+"\t"+str(start+MAXSPAN)+"\t"+str(lnct)+"_"+str(partct)+"\n");
            start = start+MAXSPAN;
            span = end - start;
            partct = partct + 1;
        out.write(cells[0]+"\t"+str(start)+"\t"+str(end)+"\t"+str(lnct)+"_"+str(partct)+"\n");
out.close();
